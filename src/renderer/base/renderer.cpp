...
bool Renderer::_CheckViewportAndScroll()
{
    ...
    COORD coordDelta;
    coordDelta.X = gsl::narrow<short>(srOldViewport.Left - srNewViewport.Left);
    coordDelta.Y = gsl::narrow<short>(srOldViewport.Top - srNewViewport.Top);
...
[[nodiscard]] std::optional<CursorOptions> Renderer::_GetCursorInfo()
{
    if (_pData->IsCursorVisible())
    {
        ...
        if (xInRange && yInRange)
        {
            ...
            options.viewportLeft = gsl::narrow<short>(_pData->GetViewport().Left());
        ...
        for (const auto& rect : dirtyAreas)
        {
            if (const auto viewDirty = rect & srCaView)
            {
                for (auto iRow = viewDirty.Top; iRow < viewDirty.Bottom; iRow++)
                {
                    ...
                    const auto source = target - til::wrap_coord(overlay.origin);
