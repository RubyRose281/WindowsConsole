...
#pragma warning(pop)
                    ...
                    for (size_t i = 0; i < a.textLength; ++i)
                    {
                        ...
                        {
                            const auto col = _api.bufferLineColumn[idx + beg];
                            GlyphPlacement placement;
                            placement.key.fontFace = mappedFontFace;
                            placement.key.glyphs.insert(placement.key.glyphs.begin(), _api.glyphIndices.begin() + beg, _api.glyphIndices.begin() + i + 1);
                            placement.baseline.x = col * _api.fontMetrics.cellSize.x;
                            placement.baseline.y = _api.lastPaintBufferLineCoord.y * _api.fontMetrics.cellSize.y + _api.fontMetrics.baseline;
                            placement.color = _api.bufferLineMetadata[col].colors.x;
                            row.emplace_back(std::move(placement));
