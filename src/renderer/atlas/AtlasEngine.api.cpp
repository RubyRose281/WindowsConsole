...
#pragma warning(suppress : 26447) // The function is declared 'noexcept' but calls function '...' which may throw exceptions (f.6).
    ...
    _resolveFontMetrics(_api.s->font->dpi, nullptr, fontInfoDesired, fontInfo);
    return S_OK;
}
CATCH_RETURN()
...
[[nodiscard]] HRESULT AtlasEngine::GetFontSize(_Out_ til::size* pFontSize) noexcept
{
    ...
    pFontSize->X = _api.s->font->cellSize.x;
    pFontSize->Y = _api.s->font->cellSize.y;
    return S_OK;
}
...
[[nodiscard]] HRESULT AtlasEngine::IsGlyphWideByFont(const std::wstring_view glyph, _Out_ bool* const pResult) noexcept
{
    ...
    RETURN_IF_FAILED(_p.dwriteFactory->CreateTextLayout(glyph.data(), gsl::narrow_cast<uint32_t>(glyph.size()), _getTextFormat(false, false), FLT_MAX, FLT_MAX, textLayout.addressof()));

    DWRITE_TEXT_METRICS metrics;
    RETURN_IF_FAILED(textLayout->GetMetrics(&metrics));

    *pResult = static_cast<unsigned int>(std::ceilf(metrics.width)) > _api.s->font->cellSize.x;
    return S_OK;
}
...
void AtlasEngine::SetAntialiasingMode(const D2D1_TEXT_ANTIALIAS_MODE antialiasingMode) noexcept
{
    ...
    if (_api.s->misc->antialiasingMode != mode)
    {
        _api.s.write()->misc.write()->antialiasingMode = mode;
    }
}
...
{
    if (_api.s->target->enableTransparentBackground != isTransparent)
    {
        _api.s.write()->target.write()->enableTransparentBackground = isTransparent;
    }
}
...
void AtlasEngine::_resolveTransparencySettings() noexcept
{
    ...
    _api.s.write()->misc.write()->antialiasingMode = _api.enableTransparentBackground && _api.antialiasingMode == D2D1_TEXT_ANTIALIAS_MODE_CLEARTYPE ? D2D1_TEXT_ANTIALIAS_MODE_GRAYSCALE : _api.antialiasingMode;
    ...
    _api.s.write()->target.write()->enableTransparentBackground = _api.enableTransparentBackground || !_api.s->misc->customPixelShaderPath.empty() || _api.s->misc->useRetroTerminalEffect;
...
void AtlasEngine::_updateFont(const wchar_t* faceName, const FontInfoDesired& fontInfoDesired, FontInfo& fontInfo, const std::unordered_map<std::wstring_view, uint32_t>& features, const std::unordered_map<std::wstring_view, float>& axes)
{
    ...
    _resolveFontMetrics(_api.s->font->dpi, faceName, fontInfoDesired, fontInfo, &*font);
    font->fontFeatures = std::move(fontFeatures);
    font->fontAxisValues = std::move(fontAxisValues);
}

void AtlasEngine::_resolveFontMetrics(u16 dpi, const wchar_t* requestedFaceName, const FontInfoDesired& fontInfoDesired, FontInfo& fontInfo, FontSettings* fontMetrics)
{
    ...
    const auto fontSizeInPx = requestedSize.Y / 72.0f * dpi;
